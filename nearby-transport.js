...
(function(){
  ...
  function showNearby(){
    ...
    var anchor=null;
    for(var i=0;i<targets.length;i++){anchor=document.querySelector(targets[i]);if(anchor)break;}
    if(!anchor){var f=document.querySelector('footer,[class*="footer"]');if(f)anchor=f;}
    if(!anchor)return;
  ...
  window.__ohcSearch=function(){
    ...
    navigator.geolocation.getCurrentPosition(function(p){
      ...
      fetch(API+'/apps/NearbyCollege/search',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({latitude:p.coords.latitude,longitude:p.coords.longitude,radius:parseInt(radius),course:course||null,limit:20})})
      .then(function(r){return r.json()})
      .then(function(d){
        if(!d.colleges||d.colleges.length===0){el.innerHTML='<div style="padding:16px;color:#94a3b8">No colleges found. Try increasing the radius.</div>';return;}
        var h='<div style="font-size:0.72rem;color:#94a3b8;margin-bottom:8px">'+d.total+' colleges within '+radius+' km</div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:10px">';
        ...
        el.innerHTML=h+'</div>';
      }).catch(function(){el.innerHTML='<div style="padding:16px;color:#f87171">Failed to search.</div>';});
